...
def extractProductName(productUrl,webpage):
    ...
    prod_names = soup.find_all('span',{'class':'a-size-base-plus a-color-base a-text-normal'})
#     print(prod_names)                         a-size-base-plus a-color-base a-text-normal
    count = 0
    for i in prod_names: 
        if count < 5:
            name = i.text
...
def extractProductPrice(productUrl,webpage):
    ...
    soup = BeautifulSoup(webpage.content,"html.parser")
    prod_prices = soup.find_all('span',{'class':'a-price-whole'})
    symbol_elem = soup.find('span', class_='a-price-symbol')
    count = 0
...
def amazon_fashion_function(search_input):
    ...
    with open("..\\Frontend\\src\\json_files\\amazon_fashion_product.json", 'w') as f:
