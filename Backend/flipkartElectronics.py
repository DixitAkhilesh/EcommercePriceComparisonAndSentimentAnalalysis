...
def extractProductName(productUrl,webpage):
    ...
    if not prod_names: 
        prod_names = soup.find_all('a',class_ = "s1Q9rs") 
    # print(prod_names)
    count = 0
    for i in prod_names: 
        if count < 5:
...
#     print(prod_urls)
    if not prod_urls:
        prod_urls = soup.find_all('a',class_='_2rpwqI')
    count = 0
    for i in prod_urls:
        if count <5:    
            prod_url = i.get("href")
...
def extractProductImage(productUrl,webpage):
    ...
    if not prods:
        prods = soup.find_all('img', {'class': 'CXW8mj'})
    count = 0
    for i in prods:
        if count < 5:
            prod_img = i.get("src")
...
def flipkart_function(search_input):
    ...
    with open("..\\Frontend\\src\\json_files\\flipkart_product.json", 'w') as f:
